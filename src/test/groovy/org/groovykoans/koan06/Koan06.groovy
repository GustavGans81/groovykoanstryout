...
class Koan06 extends GroovyTestCase {
    ...
    void test01_WithMethod() {
        ...
        String groovyResult
        // ------------ START EDITING HERE ----------------------
        StringBuilder withStringBuilder = new StringBuilder();
        groovyResult = withStringBuilder.with{
    ...
    void test03_FileIteration() {
        ...
        new File('./src').eachFileRecurse (FileType.FILES) {
            file -> if(file.text.contains('Lorem'))                    
                count++
        }
    ...
    void test04_ConcludingExercise() {
        ...
        // ------------ START EDITING HERE ----------------------
        primesBetween200And250 = (200..250).findAll { zahl ->
            (2..(zahl-1)).every { divisor ->
                zahl % divisor != 0
            }            
        }
